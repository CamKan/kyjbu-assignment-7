...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    Y = beta0 + beta1 * X + mu + np.random.normal(0, np.sqrt(sigma2), (N, 1))  # Replace with code to generate Y
    ...
    plt.plot(X, model.predict(X), color='red', label=f"Line: Y = {intercept:.2f} + {slope:.2f}X")
    plt.xlabel("X")
    plt.ylabel("Y")
    ...
    for _ in range(S):
        ...
        Y_sim = beta0 + beta1 * X_sim + mu + np.random.normal(0, np.sqrt(sigma2), (N, 1))  # Replace with code to generate simulated Y values
    ...
    slope_more_extreme = np.sum(np.array(slopes) >= slope) / S  # Replace with code to calculate proportion of slopes more extreme than observed
    intercept_extreme = np.sum(np.array(intercepts) <= intercept) / S
...
def hypothesis_test():
    ...
    # required_keys = ["N", "S", "slope", "intercept", "slopes", "intercepts", "beta0", "beta1"]
    # missing_keys = [key for key in required_keys if key not in session]
    # if missing_keys:
    #     return f"Error: Missing session variables: {', '.join(missing_keys)}. Please generate data first by clicking 'Generate Data'.", 400
    N = int(session.get("N"))
    S = int(session.get("S"))
    ...
    if p_value <= 0.0001:
        ...
        fun_message = "This is a common outcome based on the p-value."

    # TODO 12: Plot histogram of simulated statistics
    plot3_path = "static/plot3.png"
    # Plot histogram of simulated statistics
    plt.figure(figsize=(10, 5))
    plt.hist(simulated_stats, bins=20, color="lightblue", alpha=0.7, label="Distribution of Simulated Statistics")
    plt.axvline(observed_stat, color="red", linestyle="--", linewidth=2, label=f"Observed Statistic: {observed_stat:.2f}")
    plt.axvline(hypothesized_value, color="green", linestyle="--", linewidth=2, label=f"Hypothesized Value: {hypothesized_value:.2f}")
...
def confidence_interval():
    ...
    t_value = stats.t.ppf((1 + confidence_level/100) / 2, df=S - 1)
    ...
    plt.axhline(ci_lower, color="red", linestyle="--", label=f"{confidence_level}% CI Lower Bound")
    plt.axhline(ci_upper, color="green", linestyle="--", label=f"{confidence_level}% CI Upper Bound")
    plt.axhline(true_param, color="orange", linestyle=":", label=f"True Parameter Value: {true_param}")
    plt.legend()
    plt.title("Confidence Interval and Simulated Estimates")
